...
In dieser Arbeiten stellen wir ein Framework vor basierend auf \emph{Max-Flow-Min-Cut}
Berechnungen, um eine balancierte $k$-teilige Aufteilung eines Hypergraphen zu verbessern.
Varianten des \emph{FM} Algorithmus \cite{fiduccia1988linear} werden aktuell in allen modernen \emph{Multilevel Hypergraph
Partitionierer} als lokaler Suchalgorithmus eingesetzt. Solche \emph{bewegungsbasierende} Heuristiken
haben den Nachteil, dass sie nur \emph{lokale} Informationen über die Problemstruktur
in die Berechnungen miteinfließen lassen und wenn viele Knotenbewegungen den selben
Einfluss auf die Lösungsqualität haben, dann hängt das Ergebnis oft von
zufälligen Entscheidungen ab, die der Algorithmus selber trifft 
...
zwei Knoten $s$ und $t$ trennt \cite{ford1956maximal}.\\
Unser Framework ist inspierend durch die Arbeit von Sanders und Schulz \cite{sanders2011engineering}
die erfolgreich eine \emph{Fluss}-basierende Heuristik in Ihren \emph{Multilevel Graph Partitionierer}
integriert haben. Wir generalisieren viele Ihrer Ideen, sodass sie im \emph{Multilevel Hypergraph
Partitionierung}-Kontext anwendbar sind. Wir haben mehrere Techniken entwickelt, um das aktuelle 
\emph{Hypergraph Flussnetzwerk} kleiner zu machen, welche die resultierende Problemgröße um
einen Faktor $2$ reduziert, verglichen zu der aktuellen Representation \cite{lawler1973}.
Zusätzlich haben wir gezeigt, wie ein Flussproblem auf einem \emph{Subhypergraphen} konfiguriert
...
Wir haben unser Framework auf $3222$ verschiedenen Instanzen getestet. Im Vergleich zu $5$
...
Qualität der Lösungen um $2.4\%$ gestiegen, während die Laufzeit nur um einen Faktor von $2$
