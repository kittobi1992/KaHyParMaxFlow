...
\todo{refactor abstract}

Most of the multilevel hypergraph partitioner implements the \emph{FM} heuristic \cite{fiduccia1988linear}
to improve a given $k$-way partition. In each step, the \emph{FM} algorihm moves a node with
maximum \emph{gain} from its current block to an other block. The \emph{gain} of a node is
the difference of an objective function before and after moving the node. The algorithm
is easy to implement, flexible in adapting to different optimization functions and relatively fast \cite{zhao2002effective}.
However, a move is performed locally and greedily. Therefore, \emph{FM} tends to find local
optimal solutions which quality heavily depends on the \emph{initial partition} \cite{dutt1997vlsi}. 
Consequently, the probability to find a good
approximation of a global optimum drops significantly if we partition large hypergraphs.
Moreover, if we have a large number of moves with equal gain, the quality of the solution 
highly depends on random choices of the algorithm which makes the gain \emph{meaningless}
\cite{krishnamurthy1984improved,mann2014formula}. Especially in large hyperedges, we often
have many moves of vertices which gain is equal to zero if e.g., \emph{cut} is the objective function. \\
Techniques derived from the \emph{max-flow min-cut} theorem \cite{ford1956maximal} to improve
a $k$-way partition of a (hyper)graph can be seen as counterpart to the \emph{FM} heuristic.
They act \emph{globally} and are not \emph{move}-based. In this thesis, we present an alternative 
\emph{local search} technique based on \emph{Max-Flow-Min-Cut}
computations for balanced direct $k$-way hypergraph partitioning.
We integrate the work into the multilevel hypergraph partitioner \emph{KaHyPar} \cite{heuer2017improving}. 
...
We present several techniques to sparsify the state-of-the-art hypergraph flow network representation and
show that maximum flow algorithms are up to a factor of $3$ faster on our network in practice.
Further, we show how to improve the \emph{connectivity} metric of a $k$-way partition by solving a flow problem 
only on a subset of the vertices. We tested our new approach
...
produces $2\%$ better quality by a performance slowdown only by a factor of $2$. 
Moreover, our partitioner has a comparable running time to the direct $k$-way partitioner 
of \emph{hMetis} and outperforms it on $82\%$ of the benchmark instances with a
$14.71\%$ better solution quality.
