...
\begin{enumerate}
\item Present sparsifying techniques of the underlying flow network of hypergraphs
      which result in a speed up on real world benchmark instances of up to a factor of $7$.
\item We show how to configure the source and sink sets of the underlying flow network
      of a subhypergraph $H_{V'}$ ($V' \subseteq V$) such that a \emph{Max-Flow-Min-Cut}
      computation on $H_{V'}$ yields to an improvement on the original hypergraph $H$.
\item We have integrated a refinement algorithm based on \emph{Max-Flow-Min-Cut}-computations
      into the current state-of-the-art hypergraph partitioner \emph{KaHyPar} which improved
      the solution quality by $2\%$.
\item We compare \emph{KaHyPar} against other hypergraph partitioner and show that our new
      approach leads to best known solutions on most of our benchmark instances.
...
\section{Using Max-Flow-Min-Cut Computations as a Local Search Strategy}

\input{sections/local_search}
