...
(see Section \ref{sec:hybrid_network}). The following transformations are illustrated in
...
\caption{Illustration of all presented techniques to sparsify the flow network of a hypergraph.
         Transformation from \ShortT{L} to \ShortExT{H}{V} follows with Lemma \ref{lemma:node_removal}.
         Transformation from \ShortT{L} to \ShortT{G} follows with Lemma \ref{lemma:undirected_transformation}.}
...
whole maximum flow computation because $\capa_L(v,\incoming{e}) = \infty$. 
...
of \ShortExT{H}{V'} is equal to a minimum-weight $(S,T)$-cutset of $H$. 
...
\caption{Transformation of a hypergraph $H$ into the flow network \ShortExT{H}{V} by removing
...
flow network \ShortExT{H}{V'} ($V' \subseteq V$) with $S' = S \setminus V'\ \cup \bigcup\limits_{e \in I(V' \cap S)} \{\incoming{e}\}$ and 
...
$(S,T)$-flow $f_L$ of \ShortT{L} is then equal to a maximum $(S',T')$-flow $f_H$
...
of $H$ for both networks \ShortT{L} and \ShortExT{H}{V'}. Let $E''$ be the set of all
...
Consider a hypernode $v \in V$. We replace $v$ in \ShortT{L} with \emph{shortcut} edges between all
$\outgoing{e}$ and $\incoming{e}$ which are incident to $v$. The number of edges added to \ShortExT{H}{V} depends on the degree of
$v$. Each vertex $v \in V$ induces $d(v)(d(v) - 1)$ edges in \ShortExT{H}{V}. 
In \ShortT{L}, a hypernode adds $2d(v)$ edges and one additional node
...
\caption{Transformation of a hypergraph into the flow network \ShortT{G} by inserting 
...
to a maximum $(S,T)$-flow of \ShortT{L'} and \ShortT{L}. Consequently, a minimum-capacity
$(S,T)$-cutset of \ShortT{G} is equal to a minimum-weight $(S,T)$-cutset of $H$.
...
$(S,T)$-cutset of \ShortT{G} can be mapped to the corresponding hyperedge. 
Since there exists a one-one correspondence between the hypernodes
of \ShortT{L} and \ShortT{G} the corresponding bipartition consists of all hypernodes \emph{reachable}
...
into account means that \ShortT{G} is significantly smaller on high density hypergraphs
and \ShortExT{H}{V_d(3)} on low density hypergraphs. It would be preferable to combine 
