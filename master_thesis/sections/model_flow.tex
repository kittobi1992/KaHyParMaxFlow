...
calculating a minimum-capacity $(S',T')$-cutset in \ShortT{L} resp. \ShortExT{H}{V'}. 
...
second block of the bipartition. Since we delete all nodes $v \in V'$ from \ShortT{L} 
in \ShortExT{H}{V'} this relationship is no longer valid. 
...
of \ShortT{G} after removing all edges $e \in \Phi(A_L)$ of \ShortT{G}. Then there exists a path
...
we would had removed edge $\Phi(e',e'') = (v_i,v_{i+1})$ from \ShortT{G}. $P_L$ connects $S$ and
...
a $(S,T)$-cutset in \ShortT{L} after removing all edges $e \in \Phi^{-1}(A_G)$ of \ShortT{L}. Then
...
Therefore, $P_G$ is a valid path in \ShortT{G} connecting $S$ and $T$ not containing any edge of 
