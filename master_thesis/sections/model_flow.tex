...
\todo{find maybe better motivation over vertex seperator analogy}
...
in \ShortT{L}. In Section \ref{sec:source_and_sink} we apply this observation to
...
with degree smaller or equal $n$. Then our suggested flow network is denoted with \ShortExT{H}{V_d(3)}.
...
Therefore, $P_G$ is valid path in \ShortT{G} connecting $S$ and $T$, which not contains any edge in 
...
into account this means that either \ShortT{G} or \ShortExT{H}{V_d(3)} performs significantly better on
