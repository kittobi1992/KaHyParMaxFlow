...
will now present two algorithms in order to solve the maximum flow problem.
...
Goldberg and Tarjan \cite{goldberg1988new} implemented the first maximum flow algorithm
...
the following conditions: $d(s) = |V|$, $d(t) = 0$ and foreach $(u,v) \in E_f$, $d(u) \le d(v) + 1$. We say an
...
because their exist several speed up heuristics. The first one is
...
node $e$ has a capacity equal to $\omega(e)$ and each hypernode node has infinity capacity. 
Further, they showed that a minimum-weight $(s,t)$-vertex seperator in $G_*(H)$
...
except that we do not have to split the hypernodes $v \in V$. This is due to the fact, that
...
of \ShortT{L} is equal to a minimum $(s,t)$-vertex seperator of $G_*(H)$.
...
assigning the minimimum $(s,t)$-bipartition of subgraph $G_B$ to $G$. This also yields to 
...
This thesis is integrated in the $n$-level hypergraph partitioner \emph{KaHyPar}. Therefore, we
