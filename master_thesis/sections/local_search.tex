...
the flow-based refinement framework on graphs proposed by Sanders and Schulz 
\cite{sanders2011engineering} (see Section \ref{sec:flow_local_search_graph}) 
can be adapted to work in a $n$-level hypergraph partitioner, called \emph{KaHyPar}
...
(see Section \ref{sec:related_lawler}) is the flow network of $H_{V'}$ with a
...
Let $B_1$ be a bipartition of $H$ and $T_L(H_{V'})$ the flow network on subhypergraph
...
$e \notin \delta B \cup E_{\emptyset}$ of bipartition $B_1$. It is open to show that a non-cut
hyperedge $e \in \delta B$ in bipartition $B_1 = (V_1,V_2)$ cannot become a cut hyperedge in bipartition
...
Obiously, some nodes are missing in $S$ and $T$. Do understand which nodes are missing consider
...
For each hyperedge $e \in \delta B$ with $e \cap V' \subseteq V_2$ and $e \setminus V' \subseteq V_1$, 
...
present a technique with which we can generate a new flow network $T_L(H_{V' \cup V''\setminus \{v\}})$
...
$f'' := f'$. We have to distingush two cases based on a incident
hyperedge $e \in I(v)$ of a hypernode $v \in V'' \cap S$: 
...
is to define for hyperedges which are cut in $H_{V'}$ and are also in $\delta B$ the \emph{incomming}
...
modelling technique of $S$ and $T$ that we can calculate $\Delta_H$ with the help of the value 
of a maximum $(S,T)$-flow computation on $T_L(H_{V'})$. Additionaly, we demonstrate that different
modelling approaches of $S$ and $T$ which satisfy both conditions of Problem \ref{prob:ST} can lead to
different cut qualities of the minimum $(S,T)$-bipartition on the original hypergraph $H$.
...
$(s,t)$-bipartitions the same way as on \ShortT{L}.
...
a flow network \ShortT{L} such that every minimum-capacity $(s,t)$-cutset on $G$ is a 
...
using our flow-based refinement as local search algorithm on each level would 
not be applicable, because the performance slowdown would be tremendous. On this
