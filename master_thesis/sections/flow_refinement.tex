...
for a understanding which nodes are missing. Transformation $1$ illustrates our current modelling
...
We can remove $v$ from $T_L(H_{V' \cup V''})$ after applying this technique for 
all $e \in I(v)$. The cases for a vertex $v \in V'' \cap T'$ are equivalent. 
...
which we can map to a augmenting path in $T_L(H_{V' \cup V''})$ (without a proof). We can successively
...
$S''$ and $T''$ are equal with $S$ and $T$ as defined in Equation \ref{S_border_hyperedges}
...
is to define hyperedges which are cut of $H_{V'}$ and are also of $H$ the \emph{incomming}
...
an improved cut quality of the minimum $(S,T)$-bipartition of the original hypergraph $H$.
...
the \emph{connectivity} metric. We have used a similiar architecture as proposed
