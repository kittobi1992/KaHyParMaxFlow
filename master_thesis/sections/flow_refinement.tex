...
More formal:
\begin{align}
S = S_1 \cup \{\incoming{e}\ |\ e \cap V' \subseteq V_2 \ \land\ e \setminus V' \cap V_1  \neq \emptyset\} \label{S_border_hyperedges}\\
T = T_1 \cup \{\outgoing{e}\ |\ e \cap V' \subseteq V_1 \ \land\ e \setminus V' \cap V_2 \neq \emptyset \} \label{T_border_hyperedges}
\end{align}
...
$f''(u,\incoming{e}) = f''(u,\incoming{e}) + f'(v,\incoming{e})$ and $f''(s,u) = f''(s,u) + f'(v,\incoming{e})$. 
...
We define $S'' = S'' \cup \{\incoming{e}\}$. Also, we set $f''(s,\incoming{e}) = f'(v,\incoming{e})$. 
...
in $H_{V'}$, we have added the corresponding \emph{incoming hyperedge node} $\incoming{e}$
resp. \emph{outgoing hyperedge node} $\outgoing{e}$ to $S''$ resp. $T''$. Therefore, 
...
\emph{incoming} and \emph{outgoing hyperedge nodes} $\incoming{e}$ and $\outgoing{e}$ from $(S',T')$ resp.
