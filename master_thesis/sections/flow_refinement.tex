...
The idea of the proof is to extend $H_{V'}$ with all hypernodes contained in a border hyperedge 
$e \in \delta B$. Let $V''$ be the set of all hypernodes contained in a border hyperedge without $V'$. If we
add all $V'' \cap V_1$ to $S_1$ (see Equation \ref{S1_border_hyperedges}) and all $V'' \cap V_2$ to
$T_1$ (see Equation \ref{T1_border_hyperedges}) we can show that for a maximum $(S_1,T_1)$-flow 
$f'$ of $T_L(H_{V' \cup V''})$ holds that $|f'| = \omega_{H_{V'}}(B_2)$.\\
Afterwards, we use a technique to remove all $v \in V''$ of $T_L(H_{V' \cup V''})$ and show
...
\begin{proof}
$E_s$ is an arbitrary subset of $\mathcal{R}(s)$, where foreach $v \in E_s$ edge $(s,v)$ is removable.
...
we add all hypernodes which are part of a hyperedge of $H$ and also of $H_{V'}$ are fully contained
in $H_{V' \cup V''}$ the equality $|f'| = \omega_{H_{V'}}(B_2)$ holds. \\
\todo{better description of why this equality holds.}\\
In the following, we will apply Lemma \ref{lemma:general_source_and_sink_removal} on all
hypernodes $v \in V''$ such that the flow network $T_L(H_{V' \cup V''})$ converge against
$T_L(H_{V'})$ with $S$ and $T$ as source and sink set as defined in Equation 
\ref{S_border_hyperedges} and \ref{T_border_hyperedges}. Per definition a node $v \in V''$
...
a source node $\bar{s} \in e$ of $S_1$ such that edges $(s,\incoming{e})$ and $(\bar{s},\incoming{e})$
are contained in $T_L(H_{V' \cup V''})$. Therefore, $E_s$ is a removable subset of $\mathcal{R}(s)$.
...
source node $s \in V''$ then $e'$ is in the removable subset $E_s$ (see \autoref{img:delta_proof_illustration}). The same holds
for each sink node $t \in V''$. A hyperedge $e \in \delta B_2$ becomes a source node of $S$
if we remove a source node $s \in e$ and sink node of $T$ if we remove a 
