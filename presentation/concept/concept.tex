...
\begin{todolist}
\item[\done] High-level overview of framework (Mixed with related work)
    \begin{todolist}
    \item[\done] \emph{Active Block Scheduling}
    \item[\done] Build region around cut + \emph{Adaptive Flow Iterations}
    \item[\done] Solve flow problem on a hypergraph flow network
    \item[\done] \emph{Most Balanced Minimum Cut}
    \end{todolist}
\item[\done] Flow Networks
    ...
    \end{todolist}
\item[\done] Flow Problem Configuration 
    \begin{todolist}
    \item[\done] Modeling of Sanders and Schulz
    \item[\done] Optimized modeling approach
    \end{todolist}
